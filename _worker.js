...
export default {
    async fetch(request, env, ctx) {
        ...
        // ----------------- 修改点：逻辑调整，移除最外层的 if(env.KV) 包裹，允许无KV运行 -----------------
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            // 管理面板逻辑
                ...
                if (访问路径 === 'admin/log.json') {
                    // 读取日志，如果没有KV则返回空数组
                    const 读取日志内容 = (env.KV && await env.KV.get('log.json')) || '[]';
                ...
                if (访问路径 === 'admin/init') {
                    try {
                        ...
                        if(env.KV) ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Init_Config', config_JSON));
                    ...
                    // ----------------- 修改点：KV 写入前检查 -----------------
                    if (!env.KV) {
                        return new Response(JSON.stringify({ error: '未绑定KV命名空间，无法保存配置。请在 Workers 设置中绑定 KV namespace。' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                    ...
                    let 本地优选IP = (env.KV && await env.KV.get('ADD.txt')) || 'null';
                ...
                if(env.KV) ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Admin_Login', config_JSON));
                ...
                if (url.searchParams.get('token') === 订阅TOKEN) {
                    config_JSON = await 读取config_JSON(env, host, userID, env.PATH);
                    if(env.KV) ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Get_SUB', config_JSON));
                    ...
                    if (订阅类型 === 'mixed') {
                        ...
                        if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) {
                            const 完整优选列表 = config_JSON.优选订阅生成.本地IP库.随机IP ? (await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口))[0] : (env.KV && await env.KV.get('ADD.txt')) ? await 整理成数组(await env.KV.get('ADD.txt')) : (await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口))[0];
...
///////////////////////////////////////////////////////////////////////WS传输数据///////////////////////////////////////////////
// ... (WS处理相关代码保持不变，请从原文件复制 `async function 处理WS请求` 开始到 `function base64ToArray` 结束的所有内容)
...
// ... (SOCKS5/HTTP函数 和 功能性函数 保持不变，请从 `async function socks5Connect` 复制到 `async function getECH` 结束)
////////////////////////////////SOCKS5/HTTP函数///////////////////////////////////////////////
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON) {
    ...
    try {
        ...
        let 日志数组 = [];
        const 现有日志 = (env.KV && await env.KV.get('log.json'));
        if (现有日志) {
            try {
                ...
                if (config_JSON.TG.启用) {
                    try {
                        const TG_TXT = (env.KV && await env.KV.get('tg.json'));
        ...
        if(env.KV) await env.KV.put('log.json', JSON.stringify(日志数组, null, 2));
...
async function 读取config_JSON(env, hostname, userID, path, 重置配置 = false) {
    ...
    try {
        let configJSON = (env.KV && await env.KV.get('config.json'));
        if ((!configJSON && env.KV) || 重置配置 == true) {
            // 如果有KV且没有配置，或者强制重置，则保存默认配置
   ...
   // ----------------- 修改点：环境变量覆盖配置 -----------------
    ...
    if (env.SUB_REMOTE || env.SUB || env.SUBAPI) {
        ...
        config_JSON.优选订阅生成.local = false; // 强制使用远程订阅
    }
    // 处理 ECH 环境变量
    if (env.ECH) {
    ...
    try {
        const TG_TXT = (env.KV && await env.KV.get('tg.json'));
        if (!TG_TXT) {
            if(env.KV) await env.KV.put('tg.json', JSON.stringify(初始化TG_JSON, null, 2));
        } else {
            const TG_JSON = JSON.parse(TG_TXT);
    ...
    try {
        const CF_TXT = (env.KV && await env.KV.get('cf.json'));
        if (!CF_TXT) {
            if(env.KV) await env.KV.put('cf.json', JSON.stringify(初始化CF_JSON, null, 2));
        } else {
...
// ... (以下代码保持不变，请从 `async function 生成随机IP` 开始到文件结束)
